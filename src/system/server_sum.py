...
class FederatedLearningServer:
    ...
    def load_test_data_yolo_ultralytics(self, client_id, round_num=None):
        dataset_path = Path("../dataset/P33-1_Federated_Incremental/")
        client_yaml_path = dataset_path / f'client_{client_id}_round_{round_num}.yaml'
    ...
    def evaluate_model_yolo_ultralytics(self, yaml_path):
        ...
        results = model.val(data=yaml_path, imgsz=640, batch=8, device=self.device, classes=selected_class_ids, plots=True, save_json=True, verbose=False)
    ...
    def run_server(self):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            yaml_path = dataset_path = Path("../dataset/P33-1_Federated_Incremental/")
            yaml_path = dataset_path / f'client_0_round_3.yaml'
            self.global_model.train(data=yaml_path, epochs=1, batch=8, imgsz=640, device=self.device, patience=100, save_period=5,classes=selected_class_ids,val=True,plots=True, verbose=False,save_json=True)
                ...
                if client_updates:
                    ...
                    if self.args.dataset == 'COCO128':
                        ...
                        for i in self.client_idx:
                            try:
                                yaml_path = self.load_test_data_yolo_ultralytics(i, round_num=round_num)
