...
def load_train_data_yolo_ultralytics(client_id, round_num=None):
...
def evaluate_model_yolo_ultralytics(yaml_path, model_local, device):
    ...
    results = mod.val(data=yaml_path, imgsz=640, batch=8, device=device, classes=selected_class_ids, plots=True, save_json=True, verbose=False)
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        yaml_path = load_train_data_yolo_ultralytics(args.client_idx, round_num=0)
        ...
        for round_num in range(args.rounds):
            ...
            loss=[]
            try:
                yaml_path = load_train_data_yolo_ultralytics(args.client_idx, round_num=round_num)
